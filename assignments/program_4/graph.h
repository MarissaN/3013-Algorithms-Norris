...
//struct contains latitude and longitude
struct latlon
{
	double lat;
	double lon;
...
public:
	...
	*/
	...
	/**
	* graph - parameterized constructor
	*/
	graph(const graph &G)
	{
		id = G.id;
	...
	/**
	* resetGraphVisits - sets the value of visit back to false
	*/
	void resetGraphVisits() 
	{
	...
	*/
	...
	/**
	* createSpanningTree - makes a new spanning tree for the state
	* Params:
	*         string filter = the state the spanning tree is created for
	*		  int degree = the number of edges a vertex can have
	* Returns:
	*     void
	*/
	void createSpanningTree(string filter = "", int degree = 3)
	{	
		miles = 0;
		edges = 0;

		//vertor of iterators that count i, j, eit
		vector<vertex *>::iterator i;
		...
		double distance;
		int closestID, id;
		double lat1, lon1, lat2, lon2, minDistance; ;
		string state;
		...
		for (i = vertexList.begin(); i != vertexList.end(); i++)
		{
			//used to identify the city
			id = (*i)->ID;

			(*i)->E.clear();
			//loops till gets the wanted number of edges
			for (int n = 0; n < degree; ++n) 
			{
				f = false;
				//initialize minDistance biggest double a double 
				//variable can hold 
				minDistance = numeric_limits<double>::max();
				cout << (*i)->ID << endl;
				//runs from the begining of the list to the end
				for (j = vertexList.begin(); j != vertexList.end(); j++)
				{
					//check to see if the city is already used
					if (id != (*j)->ID) 
					{
						cout << (*j)->ID << endl;
						...
						//if filter is empty or if the desired state equals this the current state
						if (filter == "" || state == filter)
						{
							//if the city is the closest and not connected yet
							if (distance < minDistance && (*j)->visited == false)
							{
								minDistance = distance;
								closestID = (*j)->ID;
								miles = miles + minDistance;
								(*j)->visited = true;
								f = true;
				...
				if (f)
				{
					addEdge(id, closestID, minDistance, true);
					edges++;
				}
	...
	/**
	* printVids - prints the vertex ID
	* Params:
	*     none
	*/
	void printVids() {
		...
		//run from the begin of the list to the end
		for (vit = vertexList.begin(); vit != vertexList.end(); vit++)
		{
			cout << (*vit)->ID << endl;
		}
	...
	/**
	* graphViz - use to graph the cities in a diagram on graph viz
	* Params:
	*     void
	*/
